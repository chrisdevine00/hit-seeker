...
const DOMAIN_COLORS = {
  ...
  vp: { primary: '#22c55e', secondary: '#86efac', glow: 'rgba(34, 197, 94, 0.4)' },
  trip: { primary: '#f472b6', secondary: '#fcd34d', glow: 'rgba(168, 85, 247, 0.3)', rainbow: true },
...
export function BadgeUnlockModal({ badges, onDismiss }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {[...Array(rayCount)].map((_, i) => {
            const rainbowColors = ['#ff6b6b', '#ffd700', '#4ade80', '#60a5fa', '#a855f7', '#f472b6'];
            const useRainbow = tier === 'legendary' || domainColors.rainbow;
            const rayColor = useRainbow ? rainbowColors[i % rainbowColors.length] : domainColors.primary;
            const rayColorEnd = useRainbow ? rainbowColors[(i + 1) % rainbowColors.length] : domainColors.secondary;
            ...
            return (
              <div
                ...
                style={{
                  width: (tier === 'legendary' || domainColors.rainbow) ? '4px' : '3px',
                  height: '50%',
      ...
      <div
        ...
        style={{
          background: effectType === 'fire'
            ...
            : effectType === 'explode'
            ? undefined
            : domainColors.rainbow
            ? 'radial-gradient(circle at 50% 50%, rgba(255, 107, 107, 0.3) 0%, rgba(255, 215, 0, 0.2) 25%, rgba(74, 222, 128, 0.2) 50%, rgba(168, 85, 247, 0.2) 75%, transparent 100%)'
            : `radial-gradient(circle at 50% 50%, ${domainColors.glow} 0%, transparent 60%)`,
          opacity: showBadge && !isClosing ? tierSettings.glowIntensity : 0,
          transition: 'opacity 0.4s ease-out',
          animation: showBadge && !isClosing && effectType !== 'explode' ? 'pulse-glow 2s ease-in-out infinite' : 'none',
        }}
      />
        ...
        <>
          <div
            ...
            style={{
              ...
              opacity: isClosing ? 0 : (showBadge ? 1 : 0),
              transition: isClosing ? 'opacity 0.4s ease-out' : 'none',
              animation: showBadge && !isClosing ? 'nebula-pulse-1 2s ease-out forwards' : 'none',
            }}
          />
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              ...
              opacity: isClosing ? 0 : (showBadge ? 1 : 0),
              transition: isClosing ? 'opacity 0.4s ease-out' : 'none',
              animation: showBadge && !isClosing ? 'nebula-pulse-2 2.2s ease-out forwards' : 'none',
            }}
          />
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              ...
              opacity: isClosing ? 0 : (showBadge ? 1 : 0),
              transition: isClosing ? 'opacity 0.4s ease-out' : 'none',
              animation: showBadge && !isClosing ? 'nebula-pulse-3 1.8s ease-out forwards' : 'none',
            }}
          />
        </>
      )}
      ...
      {/* Floating sparkles */}
      {!isClosing && sparkles.map((sparkle, idx) => {
        const rainbowSparkleColors = ['#ff6b6b', '#ffd700', '#4ade80', '#60a5fa', '#a855f7', '#f472b6'];
        const sparkleColor = domainColors.rainbow
          ? rainbowSparkleColors[idx % rainbowSparkleColors.length]
          : domainColors.secondary;
      ...
      {/* Explosion effects */}
      {effectType === 'explode' && !isClosing && explosions.map(exp => exp.visible && (
        <div
          key={exp.id}
          className="absolute pointer-events-none"
          style={{
      ...
      {/* Main content */}
      <div className="text-center relative z-[100] flex flex-col items-center">
        {/* Header - fade only */}
        <div
          className="mb-2 text-sm font-semibold uppercase tracking-wider flex items-center justify-center gap-2"
          style={{
            color: domainColors.secondary,
            opacity: showBadge && !isClosing ? 1 : 0,
            transition: 'opacity 0.4s ease-out',
            transitionDelay: isClosing ? '0s' : '0.3s',
          }}
        >
        ...
        {/* Badge container - 3D spin in, fade out */}
        <div
          className="relative mx-auto mb-4"
          style={{
            width: '200px',
            height: '240px',
            transform: showBadge && !isClosing ? 'perspective(1000px) rotateY(0deg) scale(1)' : 'perspective(1000px) rotateY(180deg) scale(0.5)',
            opacity: showBadge && !isClosing ? 1 : 0,
            transition: isClosing ? 'opacity 0.4s ease-out' : 'all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)',
          }}
        >
          {/* Fire effect - positioned to engulf the badge */}
          {effectType === 'fire' && (
            <div
          ...
          {/* Hexagon badge with shimmer */}
          <div
            className="absolute left-1/2 transform -translate-x-1/2"
            style={{ zIndex: 10, top: '20px' }}
          >
            <div
              className={`w-32 h-[140px] bg-gradient-to-b ${colors.outline} flex items-center justify-center relative overflow-hidden`}
              style={{
            ...
            >
              <div
                className={`w-28 h-[123px] bg-gradient-to-b ${colors.fill} flex items-center justify-center relative overflow-hidden`}
